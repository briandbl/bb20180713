package com.ubtechinc.alpha.nuance.ttser;
...
import com.ubtechinc.alpha.nuance.nuance.NuanceConstants;
...
public final class NuanceTtser implements ITtser {
    ...
    public void init() {
        ...
        setTtsLanguage(NuanceConstants.DEFAULT_TTS_LANGUEGE);
        setTtsVoicer(NuanceConstants.DEFAULT_TTS_VOICE);
        ...
        setTtsSpeed(NuanceConstants.DEFAULT_TTS_SPEED);
        setTtsVolume(NuanceConstants.DEFAULT_TTS_VOLUME);
        setTtsPitch(NuanceConstants.DEFAULT_TTS_PITCH);
    }
    ...
    private boolean isVocalizerLanguageValid(String language) {
        for (int i = 0; i < NuanceConstants.SUPPORT_LANGS.length; i++){
            if (NuanceConstants.SUPPORT_LANGS[i].equals(language)) return true;
        }
        return false;
    }
    ...
    private boolean isVocalizerVoicerValid(String voice) {
        ...
        if (mTtsLanguage.equals(NuanceConstants.SUPPORT_LANGS[0])){
            for (VocalizerVoice v: NuanceConstants.ENU_VOICES) {
                if (v.name.equals(voice)) return true;
            }
        }else if (mTtsLanguage.equals(NuanceConstants.SUPPORT_LANGS[1])){
            for (VocalizerVoice v: NuanceConstants.RUR_VOICES) {
                if (v.name.equals(voice)) return true;
            }
        }else if (mTtsLanguage.equals(NuanceConstants.SUPPORT_LANGS[2])){
            for (VocalizerVoice v: NuanceConstants.KOK_VOICES) {
                if (v.name.equals(voice)) return true;
            }
        }
        return false;
